...
import rikser123.crawler.service.TextExtractor;
...
public class UserQueryProcessorTest extends BaseConfig {
  @Autowired
  ...
  void shouldHandleSuccessPipeline() {
    ...
    await().atMost(5, TimeUnit.SECONDS)
      ...
      .untilAsserted(() -> {
        verify(textExtractor, atLeastOnce()).extractText(argThat(arg -> {
    ...
    await().atMost(5, TimeUnit.SECONDS)
      .pollInterval(100, TimeUnit.MILLISECONDS)
      .untilAsserted(() -> {
        verify(chunkSplitter, atLeastOnce()).split(argThat(arg -> {
    ...
    await().atMost(5, TimeUnit.SECONDS)
      .pollInterval(100, TimeUnit.MILLISECONDS)
      .untilAsserted(() -> {
        verify(summariser, atLeastOnce()).summarise(argThat(arg -> {
